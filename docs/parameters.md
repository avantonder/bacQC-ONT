...
| `fastq_dir` | Path to a folder containing fastq files from the Nanopore run. | `string` |  |  |  |
...
| Parameter | Description | Type | Default | Required | Hidden |
|-----------|-----------|-----------|-----------|-----------|-----------|
| `skip_pycoqc` | Skip pycoQC (if no sequencing_summary.txt file is available) | `boolean` |  |  |  |
